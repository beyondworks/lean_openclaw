...
// ── Clips Tools ───────────────────────────────────
server.tool('list_clips', 'List saved clips with optional filters. Supports category, platform, collection, favorites, read-later, archived, date range, text search, pagination, and sorting. Use includeContent=true to get the full original text (rawMarkdown) of each clip in the response.', {
...
server.tool('get_clip', 'Get detailed information about a specific clip (title, URL, summary, keywords, notes, category, etc.).', {
    id: z.string().describe('Clip ID'),
}, async ({ id }) => safeCall(() => client.getClip(id)));
server.tool('get_clip_content', 'Get the full original content of a clip (the complete text extracted from the source URL, in markdown/HTML format).', {
    id: z.string().describe('Clip ID'),
}, async ({ id }) => safeCall(() => client.getClipContent(id)));
server.tool('search_clips', 'Search clips by keyword query. Searches across titles, summaries, keywords, notes, and content. Supports filters. Use includeContent=true to get the full original text (rawMarkdown) of each clip.', {
...
server.tool('create_clip', 'Save a new clip (bookmark) from a URL. The URL is automatically analyzed to extract title, summary, keywords, and category. You can optionally override these.', {
...
server.tool('update_clip', 'Update a clip\'s metadata (title, summary, notes, category, keywords, collections, favorite/read-later/archived status, etc.).', {
    id: z.string().describe('Clip ID'),
...
server.tool('delete_clip', 'Permanently delete a clip. Also removes it from any collections.', {
    id: z.string().describe('Clip ID to delete'),
}, async ({ id }) => safeCall(() => client.deleteClip(id)));
// ── AI Tools ──────────────────────────────────────
server.tool('generate_content', 'Generate content from clips using AI. Available types: report, planning, trend, big-picture, step-by-step, chapter-lessons, simplify, key-concepts, quiz, visual-map, review-notes, teach-back, sns-post, newsletter, presentation, email-draft, blog-post, executive-summary. Consumes credits.', {
...
server.tool('analyze_url', 'Analyze a URL using AI. Extracts title, summary, keywords, category, and other metadata. Consumes credits.', {
    url: z.string().url().describe('URL to analyze'),
}, async ({ url }) => safeCall(() => client.analyzeUrl(url)));
server.tool('ask_clips', 'Ask AI a question with optional clip context. The AI answers based on the content of specified clips. Consumes credits.', {
...
server.tool('get_insights', 'Generate an insights report analyzing reading patterns, topic trends, and recommendations over a time period. Consumes credits.', {
...
// ── Collection Tools ──────────────────────────────
server.tool('list_collections', 'List all collections (folders for organizing clips).', {}, async () => safeCall(() => client.listCollections()));
server.tool('create_collection', 'Create a new collection (folder) for organizing clips.', {
...
server.tool('update_collection', 'Update a collection\'s name, color, or visibility.', {
...
server.tool('delete_collection', 'Delete a collection. Clips in the collection are not deleted, only the collection reference is removed.', {
    id: z.string().describe('Collection ID to delete'),
}, async ({ id }) => safeCall(() => client.deleteCollection(id)));
// ── Category Tools ────────────────────────────────
server.tool('list_categories', 'List all categories with clip counts, sorted by frequency.', {}, async () => safeCall(() => client.listCategories()));
server.tool('create_category', 'Create a new custom category for organizing clips.', {
    name: z.string().describe('Category name'),
    color: z.string().optional().describe('Color hex code (e.g., "#FF5733")'),
}, async ({ name, color }) => safeCall(() => client.createCategory(name, color)));
// ── Tag Tools ─────────────────────────────────────
server.tool('list_tags', 'List all tags (keywords) used across clips, sorted by frequency. Aggregated from clip keywords.', {
    limit: z.number().min(1).max(100).optional().describe('Max tags to return'),
}, async ({ limit }) => safeCall(() => client.listTags(limit)));
server.tool('search_by_tags', 'Find clips that have specific tags/keywords. Supports AND/OR matching.', {
...
// ── Bulk Tools ────────────────────────────────────
server.tool('bulk_update', 'Perform bulk operations on multiple clips at once. Actions: delete, move (change category), tag (add keywords), favorite, archive.', {
...
// ── Webhook Tools ─────────────────────────────────
server.tool('list_webhooks', 'List all webhook subscriptions with delivery statistics.', {}, async () => safeCall(() => client.listWebhooks()));
server.tool('create_webhook', 'Create a webhook subscription. The URL must be HTTPS and reachable. Returns a secret for verifying webhook signatures (HMAC-SHA256).', {
...
server.tool('delete_webhook', 'Delete a webhook subscription.', {
