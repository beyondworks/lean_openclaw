...
import type { ThreadsReply } from "../types.js";
import type { z } from "zod";

function formatReply(r: ThreadsReply): string {
  const lines: string[] = [];
...
export function registerReplyTools(server: McpServer, client: ThreadsApiClient): void {
...
Returns:
  List of replies with text, username, timestamp, and hide status.`,
    ...
    async (params: z.infer<typeof GetRepliesSchema>) => {
      try {
        const result = await client.getReplies(params.thread_id, undefined, params.reverse);
        const replies = result.data || [];

        if (!replies.length) {
          return { content: [{ type: "text" as const, text: "No replies found for this thread." }] };
        }
...
Returns:
  Success or error message.`,
    ...
    async (params: z.infer<typeof HideReplySchema>) => {
      try {
        await client.hideReply(params.reply_id, params.hide);
        const action = params.hide ? "hidden" : "unhidden";
        return { content: [{ type: "text" as const, text: `✅ Reply ${params.reply_id} ${action} successfully.` }] };
      } catch (error) {
        return {
          content: [{ type: "text" as const, text: `Error: ${error instanceof Error ? error.message : String(error)}` }],
          isError: true,
