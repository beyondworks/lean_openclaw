...
import type { ThreadsMedia } from "../types.js";
import type { z } from "zod";

function formatThread(t: ThreadsMedia): string {
  const lines: string[] = [];
  lines.push(`**[${t.id}]** @${t.username || "me"} · ${new Date(t.timestamp).toLocaleString()}`);
  if (t.text) lines.push(t.text);
  if (t.media_type !== "TEXT_POST") lines.push(`📎 ${t.media_type}`);
  if (t.topic_tag) lines.push(`🏷️ ${t.topic_tag}`);
  if (t.permalink) lines.push(`🔗 ${t.permalink}`);
  lines.push("---");
  return lines.join("\n");
}

export function registerContentTools(server: McpServer, client: ThreadsApiClient): void {
...
Returns:
  List of your recent posts with text, media type, timestamp, and permalink.`,
    ...
    async (params: z.infer<typeof GetMyThreadsSchema>) => {
      try {
...
Returns:
  Post details including text, media type, timestamp, and permalink.`,
    ...
    async (params: z.infer<typeof GetThreadSchema>) => {
      try {
...
Returns:
  Success or error message. This action is irreversible.`,
    ...
    async (params: z.infer<typeof DeleteThreadSchema>) => {
      try {
        await client.deleteThread(params.thread_id);
        return { content: [{ type: "text" as const, text: `✅ Post ${params.thread_id} deleted successfully.` }] };
      } catch (error) {
        return {
          content: [{ type: "text" as const, text: `Error: ${error instanceof Error ? error.message : String(error)}` }],
          isError: true,
  ...
  List of matching public posts.`,
    ...
    async (params: z.infer<typeof SearchPostsSchema>) => {
      try {
        ...
        if (!posts.length) {
          return { content: [{ type: "text" as const, text: `No posts found for "${params.query}".` }] };
        }
